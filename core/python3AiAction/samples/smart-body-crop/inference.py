...
class SmartBodyCrop:
    ...
    def _download_model(self):
        ...
        if (os.path.isfile(local_model_path)):
            print_time("model was found in the local storage: " + local_model_path, start)
            return local_model_path
        
        # check if this model was downloaded by another invocation in the tmpfs path
        if (os.path.isfile(tmpfs_model_path)):
            print_time("model was found in the tmpfs storage: " + tmpfs_model_path, start)
    ...
    def infer(self, image, upper_body, lower_body):
        ...
        image, thumbnail, input_width, input_height = self.read_img(imgpath, 368, 368)
        start = print_time("image (" + str(input_width) + "x" + str(input_height) + ") loaded in: ", start)
        ...
        with tf.Session() as sess:
            ...
            img, crop_position, crop_size = crop_image(thumbnail, humans, upper_body, lower_body)
